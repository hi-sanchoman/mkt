<template>
    <div class="flex flex-col h-full">
		...
		<div class="flex justify-content justify-between">
			<h1><b>Управление весовым складом</b></h1>
        ...
        <div class="grid grid-cols-2 ">
         ...
         <table class="w-full whitespace-nowrap mt-5">
                   ...
                   <tr v-for="action in myactions" class="w-full whitespace-nowrap mt-5 tableizer-table" v-if=" from[0] && from[0].length == 0 && action.t_to.toLowerCase().includes(poisk.toLowerCase())">
                        <!--
                            <tr v-for="action in myactions" class="w-full whitespace-nowrap mt-5 tableizer-table" v-if=" action.t_to.toLowerCase().includes(poisk.toLowerCase()) && new Date(action.created_at) >= new Date(from[0]) && new Date(action.created_at) <= new Date(from[1])">
                        -->
