<template>
  <div class="flex flex-col h-full">
    ...
    <div class="panel grid grid-cols-2 hidden sm:flex justify-start items-center gap-5">
      ...
      <div v-if="dops.length > 0" class="bg-blue-100 border-t border-b border-blue-500 text-blue-700 px-4 py-3 cursor-pointer" role="alert" @click="showDops()">
    ...
    <div class="sm:hidden">
      <div v-if="canApply <= 0 || myrealizations.length <= 0" class="mt-3">
        <a class="bg-blue-500 hover:bg-blue-700 text-white font-bold mt-3 py-2 px-4 rounded" href="/realizators/new-order">Новая заявка</a>
      </div>
      <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold mt-3 py-2 px-4 rounded mb-3" @click="$modal.show('nak_history')">история накладных</button>
    </div>
    ...
    <div class="sm:hidden">
      ...
      <div v-if="myrealizations[0]" class="text-sm mt-3">Дата заявки: {{ moment(myrealizations[0].created_at).format('DD.MM.YYYY HH:mm') }}</div>
      ...
      <div v-if="myrealizations[0]" class="w-full whitespace-nowrap mt-4">
        ...
        <div v-for="(item1, key1) in myrealizations[0].order" :key="key1 + '-0'" :id="key1 + '-0'" class="bg-white shadow rounded-lg">
      ...
      <div class="mt-3 font-bold" style="margin-top: 40px">НОВАЯ ЗАЯВКА</div>

      <div v-if="myrealizations[1]" class="text-sm mt-3">Дата заявки: {{ formatDate(myrealizations[1].created_at) }}</div>
      ...
      <div v-if="myrealizations[1]" class="w-full whitespace-nowrap">
        ...
        <div v-for="(item1, key1) in myrealizations[1].order" :key="key1" :id="key1 + '-1'" class="bg-white shadow rounded-lg">
          <template v-if="assortment[item1.assortment_id]">
            <div class="bg-white p-1 rounded-md mb-1 flex flex-row w-full">
              <span class="mr-1" style="width: 20px; font-size: 0.65rem">#{{ key1 + 1 }}:</span>
              <span class="flex-1" style="font-size: 0.65rem; width: 30%; overflow: hidden">{{ assortment[item1.assortment_id].type }}</span>
    ...
    <div v-if="report" class="w-full bg-white rounded-2xl h-auto p-6 overflow-auto pt-2 hidden sm:block md:mt-4">
      <div v-if="myrealizations[0]" class="text-bold mt-2">Дата заявки: {{ moment(myrealizations[0].created_at).format('DD.MM.YYYY HH:mm') }}</div>
    ...
    <div v-if="nakladnye">
      ...
      <div class="panel py-6">
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" @click="saveNakladnoe()">сохранить</button>
        <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" @click="$modal.show('nak_history')">история</button>
      </div>
    </div>
    ...
    <modal name="myorder1">
      <form id="dop_order_form" onsubmit="return false;">
        <div class="p-6">
          <h5 class="font-bold text-lg">Ассортимент</h5>
          <table class="w-full whitespace-nowrap mt-5">
            <tr class="text-center font-bold border-b border-gray-200 py-2">
              <th class="text-left py-2">#</th>
              <th class="text-left py-2">Ассортимент</th>
          ...
          <tr class="text-left border-b border-gray-200" v-for="item in auth_realization" v-if="new Date(from) <= new Date(item.created_at) && new Date(to) >= new Date(item.created_at)">
            ...
            <td class="px-6 pt-3 pb-3 w-8">
              <div class="flex gap-2">
                <button v-if="$page.props.auth.user.position_id != 3" @click="showReport3(item.id, item.realizator ? item.realizator.id : 0, item)" class="bg-green-500 text-white font-bold py-2 px-4 rounded">редактировать</button>
              </div>
            </td>
          </tr>
        </table>
      ...
      <div class="px-6 py-6 overflow-auto max-sm:text-xs">
        ...
        <table class="w-full mt-3 min-w-640px">
          ...
          <tr class="border" v-for="nak in nakladnoe" :key="nak.id">
            ...
            <td class="text-left px-2 py-2 hover:text-blue-500" @click="showNakladnaya(nak.id)">
              Накладная для <span v-if="nak.shop != null" class="underline">{{ nak.shop.name }}</span>
            </td>
            <td class="text-left px-2 py-2">{{ moment(nak.created_at).format('DD.MM.YYYY H:mm') }}</td>
            <td class="text-left px-2 py-2">{{ nak.sum }}</td>
            ...
            <td class="text-left px-2 py-2">
              <a class="border rounded px-1 py-1 hover:bg-blue-400 hover:text-white text-xs" :href="'/blank/' + nak.id"> Скачать </a>
            </td>
          </tr>
        </table>
...
export default {
  ...
  methods: {
    ...
    fetchNakladnye(page) {
      let search = this.search_marketname

      axios.get('realizator-nakladnye?page=' + page + (search !== '' ? `&search=${search}` : '')).then((response) => {
    ...
    totalRealization() {
      ...
      if (this.columns != null) {
        this.columns.forEach((element) => {
          if (element != 0) total = total + parseInt(element.amount)
        })
      }
    ...
    getRealizationSum() {
      ...
      if (this.columns != null) {
        this.columns.forEach((element) => {
          if (element != null && element.isNal == false) total = total + parseInt(element.amount)
        })
      }
