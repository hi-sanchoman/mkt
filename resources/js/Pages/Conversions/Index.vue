<template>
  <div class="flex flex-col h-full">
    ...
    <div class="grid grid-cols-2 sm:flex panel justify-start gap-3 hidden sm:block mb-3">
      <button v-if="real && this.$page.props.auth.user.position_id != 7" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" @click="showInput()">Новая переработка</button>

      <button class="hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" :class="{ 'bg-green-500': real, 'bg-blue-500': !real }" @click="showReport">Помесячный отчет</button>
      ...
      <button v-if="this.$page.props.auth.user.position_id != 7" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" @click="endMonth()">Завершить месяц</button>
    </div>
      ...
      <table class="w-full whitespace-nowrap text-xs sm:text-base">
        <thead class="bg-white custom1">
          <tr class="text-left font-bold border-b border-gray-200 bg-white">
            ...
            <td
              v-for="(n, i) in days"
              class="px-6 pt-4 pb-4 sticky top-0 bg-white"
              :class="{
        ...
        <tbody>
          <tr v-for="(item, j) in assortments">
            <th class="pl-6 sm:pl-7 pr-6 pt-4 pb-4 text-left sticky left-0 bg-white w-72 sm:w-auto">{{ item.name }}</th>
            <td
              v-for="(n, i) in days"
    ...
    <div v-if="input" class="w-auto bg-white rounded-2xl h-auto p-3 overflow-auto pt-2">
      <form onsubmit="return false;">
        ...
        <table class="w-full whitespace-nowrap" v-if="current_month">
          ...
          <tr v-for="(item, a) in assortments" v-if="item.id != 25" :class="([1,2,3].includes(item.id) ? ' bg-orange-100' : (a % 2 === 0 ? 'bg-gray-100' : 'bg-white'))">
            ...
            <td v-else-if="getItem(item.id) && $page.props.auth.user.position_id == 1" class="px-6 pt-4 pb-4">
              <input type="text" class="pt-2 pb-2 border-b-2" v-on:keyup.enter="onEnter" onclick="select()" :name="item.assortment" :id="item.id" v-model="conversion[item.id]" />
              <span class="ml-2">в базе: {{ getItem(item.id).kg }}</span>
            </td>

            <!-- SAVED -->
            <td v-else-if="getItem(item.id).status != null && $page.props.auth.user.position_id != 1" class="px-6 pt-4 pb-4">
              {{ getItem(item.id).kg }}
            </td>
            ...
            <td v-else class="px-6 pt-4 pb-4">
              <input v-if="inMilk(item.id) && isInTime()" type="text" class="pt-2 pb-2 border-b-2" v-on:keyup.enter="onEnter" onclick="select()" :name="item.assortment" :id="'m' + item.id" v-model="dopMilk[item.id]" />
            ...
            <td>
              <button class="btn px-1 py-1 rounded" style="outline: 1px solid #ddd" v-if="inZakvaska(item.id)" @click="showZakvaska(item)">посм. закваски</button>
            </td>
          </tr>
    ...
    <!-- sidebar Закваска -->
    <div class="sidebar fixed w-screen h-screen overflow-hidden flex bg-indigo-500 bg-opacity-40 left-0" :class="[sidebar_zakvaska ? 'left-0' : 'left-full']" v-if="sidebar_zakvaska">
      ...
      <div class="w-2/5 bg-white overflow-y-auto">
        <div class="mb-8 flex justify-start w-full p-8 mr-2 items-center border-b">
          <h2>
            Информация о закваске: <strong>{{ sidebar_zakvaska_name }}</strong>
          </h2>
        </div>
...
export default {
  ...
  methods: {
    ...
    existsAssortmentDay(assortment_id, day) {
      return this.myrows[assortment_id] !== undefined && this.myrows[assortment_id][day] !== undefined ? true : false
    },

    getKilo(assortment_id, day) {
      return this.myrows[assortment_id] !== undefined && this.myrows[assortment_id][day] !== undefined ? Math.round(this.myrows[assortment_id][day] * 100) / 100 : 0
    },
