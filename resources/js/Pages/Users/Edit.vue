  ...
  <div class="overflow-y-auto">
    <div class="mb-8 flex justify-start w-full p-8 mr-2 items-center border-b">
      ...
      <h1 class="font-bold text-xl">
        ...
        <p class="text-sm">{{ user.position.name }} ыыы</p>
      </h1>
    </div>
    ...
    <div class="bg-white rounded-md overflow-hidden w-full px-8">
      ...
      <div class="flex flex-wrap mb-3 justify-start mt-3" v-if="userIs([DIRECTOR])">
        <div class="w-3/12 space-y-6 py-2">
          <p class="font-medium">Заявки до</p>
        </div>
        <div class="w-9/12 space-y-4 pl-5 py-2">
          <div class="flex items-center bg-white h-6">
            <input type="time" class="pb-1" :value="time" @change="updateTime" />
            <div class="px-1 h-[22px] pb-2 cursor-pointer hover:text-blue-400" @click="updateRequestBeforeTime"><i class="fa fa-save" /></div>
          </div>
        </div>
      </div>
...
export default {
  ...
  created() {
    ...
    axios.get('/get-profile/' + this.$page.props.actions.selected_user).then((response) => {
