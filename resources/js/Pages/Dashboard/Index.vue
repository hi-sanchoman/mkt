<template>
  <div class="sm:flex flex-col h-full">
    <div class="grid grid-cols-1 sm:flex panel justify-between">
      <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded hidden sm:block" @click="openCreateModal()">Новая поставка</button>

      <div class="bg-green-500 h-8 text-white font-bold py-2 px-4 rounded">
        {{ today() }}
      </div>

      <div class="max-lg:space-y-3 w-2/3 sm:flex gap-5 items-center">
        ...
        <label class="inline-flex items-center">
          <select v-model="month" @change="fullMonth($event.target.selectedIndex)" class="h-34px rounded px-2">
        ...
        <datepicker v-model="mydate" type="date" placeholder="" :show-time-header="time" range @change="setDay()"> </datepicker>
      </div>
    </div>
    <br />
    ...
    <div class="w-full bg-white rounded-2xl h-auto overflow-y-auto hidden sm:block">
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold border-b border-gray-200">
          ...
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Кислотность</p>
          </th>
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Плотность</p>
          </th>
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Базовый вес</p>
          </th>
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Жир килограмм</p>
          </th>
          <th class="px-6 pt-4 pb-4">
          ...
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Время</p>
          </th>
          <th class="px-6 pt-4 pb-4">
            <p class="font-bold text-center">Принимающий</p>
          </th>
        </tr>
    ...
    <modal name="create" class="modal-50">
      <form class="py-6 px-6 bg-white rounded-lg overflow-y-auto overflow-x-hidden h-full" onsubmit="return false;">
        <div class="mb-8 font-medium">Новая поставка</div>
        <div class="space-y-4 mb-8">
          <div class="flex items-end">
            ...
            <a class="w-3/12 bg-blue-500 hover:bg-blue-700 text-white text-center font-bold py-2 px-4 rounded ml-3" :href="route('create-supplier')"> Новый поставщик </a>
          </div>
          ...
          <div>
            <p class="w-1/6">Дата<span class="text-red-400">*</span></p>
            <input type="date" onclick="select()" class="flex-auto border-b-2 w-full pb-1" v-model="form.date" />
          </div>
        </div>
        ...
        <table class="w-full whitespace-nowrap">
          <tr class="text-left font-bold border-b border-gray-200">
            ...
            <th v-if="$page.props.auth.user.position_id == 1" class="px-6 pt-4 pb-4">
              <p class="font-bold text-center">Операции</p>
            </th>
          </tr>
          ...
          <tr v-for="supply in mysupplies" class="text-center hover:bg-gray-100 focus-within:bg-gray-100 mb-3" :key="supply.id">
            ...
            <td class="px-6 pt-3 pb-3 w-8">
              <p class="text-sm">{{ Math.round((supply.basic_weight) * 100) / 100 }}</p>
            </td>
            ...
            <td class="px-6 pt-3 pb-3 w-8">
              <p class="text-sm">{{ Math.round((supply.sum + Number.EPSILON) * 100) / 100 }}</p>
            </td>
